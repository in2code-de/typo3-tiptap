...
function Ke(t = []) {
  ...
  i(c, "#"), A(c, Dt, c), i(c, We, c);
  const m = i(c, Fe);
  i(m, "#"), A(m, Dt, c);
  const g = [[I, a], [P, s]], y = [[I, null], [W, l], [P, r]];
...
function Ce(t, n) {
  ...
  let r = 0, a = 0;
  for (; a < o; ) {
    let l = t, f = null, d = 0, h = null, c = -1, m = -1;
    for (; a < o && (f = l.go(e[a])); )
      l = f, l.accepts() ? (c = 0, m = 0, h = l) : c >= 0 && (c += e[a].length, m++), d += e[a].length, r += e[a].length, a++;
    r -= c, a -= m, d -= c, s.push({
      t: h.t,
      // token type/name
      v: n.slice(r - d, r),
...
Re.prototype = {
  ...
  */
   ...
   */
  render(t) {
    const n = this, e = this.toHref(t.get("defaultProtocol")), o = t.get("formatHref", e, this), s = t.get("tagName", e, n), r = this.toFormattedString(t), a = {}, l = t.get("className", e, n), f = t.get("target", e, n), d = t.get("rel", e, n), h = t.getObj("attributes", e, n), c = t.getObj("events", e, n);
    return a.href = o, l && (a.class = l), f && (a.target = f), d && (a.rel = d), h && Object.assign(a, h), {
      tagName: s,
      attributes: a,
      content: r,
      eventListeners: c
...
function $e({
  ...
  const n = t.domain.concat([gt, At, w, kt, Lt, Tt, Ct, Rt, b, Vt, $, yt, Ot, bt, N, Nt, q, St]), e = [Et, x, Xt, S, Zt, $, Q, te, ee, rt, at, _, G, ot, nt, st, it, lt, ct, ut, dt, ht, ft, pt, mt], o = [gt, Et, At, kt, Lt, Tt, Ct, Rt, b, _, G, $, yt, Ot, bt, Q, N, Nt, q, St], s = O(), r = i(s, q);
  u(r, o, r), u(r, t.domain, r);
  const a = O(), l = O(), f = O();
  u(s, t.domain, a), u(s, t.scheme, l), u(s, t.slashscheme, f), u(a, o, r), u(a, t.domain, a);
  const d = i(a, w);
  i(r, w, d), i(l, w, d), i(f, w, d);
  const h = i(r, S);
  u(h, o, r), u(h, t.domain, r);
  const c = O();
  u(d, t.domain, c), u(c, t.domain, c);
  const m = i(c, S);
  u(m, t.domain, c);
  const g = O(fe);
  u(m, t.tld, g), u(m, t.utld, g), i(d, J, g);
  const y = i(c, b);
  i(y, b, y), u(y, t.domain, c), u(g, t.domain, c), i(g, S, m), i(g, b, y);
  const k = i(g, x);
  u(k, t.numeric, fe);
  const T = i(a, b), p = i(a, S);
  i(T, b, T), u(T, t.domain, a), u(p, o, r), u(p, t.domain, a);
  const L = O(tt);
  u(p, t.tld, L), u(p, t.utld, L), u(L, t.domain, a), u(L, o, r), i(L, S, p), i(L, b, T), i(L, w, d);
  const Oe = i(L, x), ie = O(tt);
  u(Oe, t.numeric, ie);
  const C = O(tt), B = O();
  u(C, n, C), u(C, e, B), u(B, n, C), u(B, e, B), i(L, N, C), i(ie, N, C);
  const Pt = i(l, x), be = i(f, x), Se = i(be, N), vt = i(Se, N);
  u(l, t.domain, a), i(l, S, p), i(l, b, T), u(f, t.domain, a), i(f, S, p), i(f, b, T), u(Pt, t.domain, C), i(Pt, N, C), i(Pt, Q, C), u(vt, t.domain, C), u(vt, n, C), i(vt, N, C);
  const re = [
    [_, G],
    // {}
    [nt, ot],
  ...
  for (let Ht = 0; Ht < re.length; Ht++) {
    ...
    u(Y, n, U);
    const z = O();
    u(Y, e), u(U, n, U), u(U, e, z), u(z, n, U), u(z, e, z), i(U, wt, C), i(z, wt, C);
  }
...
function Qe(t, n, e) {
  ...
  for (; s < o; ) {
    let l = t, f = null, d = null, h = 0, c = null, m = -1;
    for (; s < o && !(f = l.go(e[s].t)); )
      a.push(e[s++]);
    for (; s < o && (d = f || l.go(e[s].t)); )
      f = null, l = d, l.accepts() ? (m = 0, c = l) : m >= 0 && m++, s++, h++;
    if (m < 0)
      s -= h, s < o && (a.push(e[s]), s++);
    else {
      a.length > 0 && (r.push(Bt(pe, n, a)), a = []), s -= m, h -= m;
      const g = c.t, y = e.slice(s - h, s);
      r.push(Bt(g, n, y));
    }
  }
...
function nn(t) {
  return new qt({
    ...
    appendTransaction: (n, e, o) => {
      const s = n.some((d) => d.docChanged) && !e.doc.eq(o.doc), r = n.some((d) => d.getMeta("preventAutolink"));
      if (!s || r)
        return;
      const { tr: a } = o, l = Pe(e.doc, [...n]);
      if (ve(l).forEach(({ newRange: d }) => {
        const h = He(o.doc, d, (g) => g.isTextblock);
        let c, m;
        if (h.length > 1)
          c = h[0], m = o.doc.textBetween(
            c.pos,
          ...
          const g = o.doc.textBetween(d.from, d.to, " ", " ");
          if (!Ze.test(g))
            return;
          c = h[0], m = o.doc.textBetween(c.pos, d.to, void 0, " ");
        }
        if (c && m) {
          const g = m.split(Xe).filter(Boolean);
          if (g.length <= 0)
            return !1;
          const y = g[g.length - 1], k = c.pos + m.lastIndexOf(y);
          if (!y)
            return !1;
    ...
    props: {
      handleClick: (n, e, o) => {
        ...
        if (t.enableClickSelection && (a = t.editor.commands.extendMarkRange(t.type.name)), t.openOnClick) {
          ...
          if (!l)
            return a;
          const f = xe(n.state, t.type.name), d = (s = l?.href) != null ? s : f.href, h = (r = l?.target) != null ? r : f.target;
          l && d && (window.open(d, h), a = !0);
        }
        return a;
      }
    }
  ...
  return new qt({
    ...
    props: {
      handlePaste: (n, e, o) => {
        ...
        const d = ye(f, { defaultProtocol: t.defaultProtocol }).find(
          (h) => h.isLink && h.value === f
        );
        return !f || !d || s !== void 0 && !s(d.href) ? !1 : t.editor.commands.setMark(t.type, {
          href: d.href
        });
      }
    }
